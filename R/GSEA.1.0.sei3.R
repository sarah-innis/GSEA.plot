...
GSEA.GeneRanking <- function(A, class.labels, gene.labels, nperm, permutation.type = 0, sigma.correction = "GeneCluster", fraction=1.0, replace=F, reverse.sign= F,abs.val=F) {
...
# compute S2N for the random permutation matrix
     ...
     rm(S2)
     gc()
      #typically sorts with decreasing signal to noise-- if reverse is true then reverse it
     s2n.matrix <- M1/S1
...
# compute S2N for the "observed" permutation matrix
     ...
     # HERE IS WHERE S2N IS CALCULATED
     M1 <- M1 - M2
     rm(M2)
     gc()
     S1 <- S1 + S2
     ...
     #making this absolute value to see what happens
     if (abs.val==T){
...
# its use, misuse, or functionality.
  ...
  # a RES file has description, then accession, then sample, then absent or present and repeat
  #it's number of samples is [3,1]
  #data table starts [4,1]

   # header.cont <- readLines(filename, n = 1)
   # temp <- unlist(strsplit(header.cont, "\t"))
   # colst <- length(temp)
   # header.labels <- temp[seq(3, colst, 2)]
   # #changed row names from 2 to 1
   # ds <- read.delim(filename, header=F, row.names = 1, sep="\t", skip=3, blank.lines.skip=T, comment.char="", as.is=T)
   # colst <- length(ds[1,])
   # cols <- (colst - 1)/2
   # rows <- length(ds[,1])
...
# its use, misuse, or functionality.
      ...
      #I don't have a *name column name so I'm changing it to -1. I'm going to try undoing this
      #col.names <- col.names[-1]
      col.names <- col.names[c(-1, -2)]
      num.cols <- length(col.names)
      content <- content[-1]
      #have 230001 lines for some reason
...
# preproc.type control the type of pre-processing: threshold, variation filter, normalization
 ...
 if (regexpr(pattern=".gmt", gs.db[1]) == -1) {
      ...
      names(gene.set.reference.matrix) <- gene.set.reference
      Ng <- gs.count - 1
      ...
      out4 <- vector("list", length=length(gs.names))
      out5=vector("list",length=length(gs.names))
      out6=vector("list",length=length(gs.names))
      out7=vector("list", length=length(gs.names))
...
# Read gene and gene set annotations if gene annotation file was provided
 ...
 for (nk in 1:n.tot) {
  ...
  for (i in 1:Ng) {
       ...
       #signal strength- does not appear to be calculated as signal to noise?
...
# plot S2N correlation profile
     ...
     #absolute value here-- this is when they are plotting signal to noise
...
GSEA.Analyze.Sets <- function(
   ...
   #directory="C:/Users/Student/Documents/CivelekLab/FatFC/GSEA-P-R/Leukemia_C1b/"
   ...
   for (i in 1:max.sets) {
      ...
      #phenotype is .
      phenotype <- temp3[[1]][1]
      #seq.number is currently M think it should probably be 3
